...
# Вам требуется написать программу, которая проверяет счастливость билета  (без циклов).
# *Пример:*
# 385916 -> yes
# 123456 -> no

number = input("Введите номер билета (6 цифр): ").strip()
if len(number) != 6:
    print("Длина номера должна быть равна 6!")
else:
